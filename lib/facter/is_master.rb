...
def get_options_from_hash_config(config)
  ...
  result << "--authenticationDatabase '$external' --authenticationMechanism MONGODB-X509" unless config['setParameter'].nil? || ! config['setParameter']['authenticationMechanisms'].include? 'MONGODB-X509'
